...
export default function SermonDetailPage() {
  ...
  if (!id) return <Navigate to="/sermons" replace />;
  if (!isLoading && !isError && !data) return <Navigate to="/not-found" replace />;

  if (isLoading) {
    return (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div className="rounded-3xl border border-white/10 bg-white/5 h-[420px] animate-pulse" />
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white/80">
          Unable to load sermon right now.
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          {sermon.description ? (
            <p className="mt-5 max-w-3xl text-white/80 leading-relaxed">{sermon.description}</p>
          ) : null}
        </div>
      </div>
    </div>
